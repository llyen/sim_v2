...
class CountersController extends Controller
{
	...
	{
		$data = Yii::app()->db->createCommand('select cp.symbol as collection_point, m.name as medium, c.id, c.symbol, c.installation_date, c.archival from counters c join collection_points cp on c.collection_point_id=cp.id join mediums m on c.medium_id=m.id where c.archival = 0 and c.collection_point_id in (select cp.id from collection_points cp where cp.object_id in (select o.id from objects o where o.unit_id = '.Yii::app()->user->getState('unit_id').'))')->queryAll();
